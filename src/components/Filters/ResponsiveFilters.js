...
const ResponsiveFilters = ({
  ...
  // Load real topics and sources from Directus CMS
  useEffect(() => {
    const loadRealData = async () => {
      try {
        const [topicsData, sourcesData] = await Promise.all([
          directusService.getTopics(),
          directusService.getSources()
        ]);
        
        // Extract topic names from Directus data
        const topicNames = topicsData.map(topic => topic.name || topic.slug).filter(Boolean);
        setRealTopics(topicNames);
        
        // Extract source names from Directus data
        const sourceNames = sourcesData.map(source => source.name).filter(Boolean);
        setRealSources(sourceNames);
      } catch (error) {
        console.error('Failed to load topics and sources:', error);
        // Fallback to provided props if API fails
        setRealTopics(availableTopics);
        setRealSources(availableSources);
      }
    };

    loadRealData();
  }, [availableTopics, availableSources]);
  ...
  const FilterContent = ({ isMobile = false }) => (
    <div className="space-y-4">
      ...
      {/* Topic Filter - Use real Directus data */}
      <FilterChips
        label="Topics"
        options={realTopics.length > 0 ? realTopics : availableTopics}
        selectedValue={filterTopic}
        onChange={onTopicChange}
        multiSelect={true}
      />

      {/* Source Filter - Use real Directus data */}
      {(realSources.length > 0 || availableSources.length > 0) && (
        <div>
          <label className="block text-sm font-medium text-text-primary-light dark:text-text-primary-dark mb-2">
            News Sources
          </label>
          <div className="max-h-32 overflow-y-auto space-y-2 border border-primary-200 dark:border-primary-700 rounded-lg p-3">
            {(realSources.length > 0 ? realSources : availableSources).map((source) => (
      ...
      <div className="bg-surface-light/95 dark:bg-surface-dark/95 backdrop-blur-sm border-b border-primary-200 dark:border-primary-800 p-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="space-y-4">
            ...
            {(realTopics.length > 0 || availableTopics.length > 0) && (
              <FilterChips
                options={(realTopics.length > 0 ? realTopics : availableTopics).map(topic => ({ value: topic, label: topic }))}
                selectedValue={filterTopic}
                onChange={onTopicChange}
                label="Topics"
                multiSelect={true}
