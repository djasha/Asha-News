...
const CompactArticleCard = ({ article, showImage = false, showBias = true, onClick }) => {
  ...
  const handleClick = () => {
    if (onClick) {
      ...
      // Determine if it's an internal article or external
      const articleId = article.id || article._id || article.guid;
      const isInternalId = typeof articleId === 'string'
        ? articleId.startsWith('cms-') || /^(\d+)$/.test(articleId)
        : (typeof articleId === 'number');
      
      if (isInternalId) {
        // Navigate to internal article page
        navigate(`/article/${articleId}`);
      } else if (article.url || article.source_url) {
        // Open external article in new tab
        window.open(article.url || article.source_url, '_blank', 'noopener,noreferrer');
      } else {
