...
const ArticleCard = ({ 
  ...
  return (
    ...
    >
      {/* Image */}
      {article.image_url && (
        <div className="mb-3 -mx-3 -mt-3 overflow-hidden rounded-t-lg">
          <img 
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-text-secondary-light dark:text-text-secondary-dark">
              {formatDate(article.publication_date)} • {estimateReadingTime(article.summary)} min read
            </p>
          </div>
        </div>
          ...
          <div className="relative">
            ...
            {showShareMenu && (
              <div className="absolute right-0 top-full mt-1 bg-surface-light dark:bg-surface-dark border border-primary-200 dark:border-primary-700 rounded-lg shadow-lg py-1 z-10 min-w-[120px]">
                <button onClick={() => handleShare('twitter')} className="w-full px-3 py-2 text-left text-xs text-text-primary-light dark:text-text-primary-dark hover:bg-primary-50 dark:hover:bg-primary-900 transition-colors">Twitter</button>
                <button onClick={() => handleShare('facebook')} className="w-full px-3 py-2 text-left text-xs text-text-primary-light dark:text-text-primary-dark hover:bg-primary-50 dark:hover:bg-primary-900 transition-colors">Facebook</button>
                <button onClick={() => handleShare('linkedin')} className="w-full px-3 py-2 text-left text-xs text-text-primary-light dark:text-text-primary-dark hover:bg-primary-50 dark:hover:bg-primary-900 transition-colors">LinkedIn</button>
                <button onClick={() => handleShare('copy')} className="w-full px-3 py-2 text-left text-xs text-text-primary-light dark:text-text-primary-dark hover:bg-primary-50 dark:hover:bg-primary-900 transition-colors">Copy Link</button>
              </div>
            )}
          </div>
      ...
      <div className="space-y-2">
        ...
        {/* Additional metadata - always visible in card view */}
        <div className="pt-3 border-t border-primary-200 dark:border-primary-700 space-y-3">
          ...
          <a 
            ...
            className="inline-block text-xs text-primary-500 dark:text-primary-400 hover:text-primary-600 dark:hover:text-primary-300 underline"
