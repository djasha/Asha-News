...
const StoryCluster = ({ cluster, onArticleClick }) => {
  ...
  const { trackStoryClusterView, trackEngagement } = useAnalytics();
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => {
              ...
              // Track story cluster view when expanded
              if (newExpanded) {
                trackStoryClusterView({
                  id: cluster.id || `cluster-${mainArticle.id}`,
              ...
              // Track engagement
              trackEngagement('story_cluster_expand', {
                action: newExpanded ? 'expand' : 'collapse',
                cluster_id: cluster.id || `cluster-${mainArticle.id}`,
          ...
          <button
            onClick={() => {
              // Navigate to story cluster page
              window.location.href = `/story/${cluster.id || `cluster-${mainArticle.id}`}`;
            }}
