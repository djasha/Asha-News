...
const GroundNewsStyleCluster = ({ cluster, onArticleClick }) => {
  ...
  const { mainArticle, articles, sources, biasDistribution: rawBiasDistribution, isBlindspot } = cluster;
  ...
  const biasDistribution = {
    left: extractPercentage(rawBiasDistribution?.left),
    center: extractPercentage(rawBiasDistribution?.center),
    right: extractPercentage(rawBiasDistribution?.right)
  };

  const handleViewStory = () => {
    // Navigate to the story cluster page instead of individual article
    window.location.href = `/story/${cluster.id}`;
  };

  const buildKeyTakeaways = () => {
    if (!Array.isArray(articles) || articles.length === 0) return [];
    // Simple heuristic: take top 3 article titles and normalize
    const points = articles.slice(0, 3).map(a => a.title)
      .filter(Boolean)
