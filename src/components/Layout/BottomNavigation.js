...
const BottomNavigation = () => {
  ...
  const { data: menuData } = useMenuSettings('mobile');
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center py-2 px-3 rounded-lg transition-all duration-200 touch-manipulation min-w-0 flex-1 ${
                isActive
                  ? 'text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20'
                  : 'text-text-secondary-light dark:text-text-secondary-dark hover:text-primary-600 dark:hover:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/10'
              }`}
              aria-label={item.label}
            >
              <div className={`transition-transform duration-200 ${isActive ? 'scale-110' : ''}`}>
                {iconMap[item.icon] || iconMap['circle']}
              </div>
              <span className={`text-xs mt-1 font-medium truncate ${isActive ? 'font-semibold' : ''}`}>
                {item.label}
              </span>
            </button>
          );
