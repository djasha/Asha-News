...
const DailyBriefs = ({ articles = [] }) => {
  const [cmsBriefs, setCmsBriefs] = useState([]);

  // Fetch daily briefs from CMS
  useEffect(() => {
    const fetchDailyBriefs = async () => {
      try {
        const response = await fetch('/api/cms/daily-briefs');
        const data = await response.json();
        setCmsBriefs(data.data || []);
      } catch (error) {
  ...
  // Fallback brief types with articles
  const fallbackBriefTypes = [
    {
      id: 'morning',
      title: 'Morning Brief',
      icon: (
  ...
  // Use CMS briefs if available, otherwise fallback
  const briefTypes = cmsBriefs.length > 0 ? cmsBriefs.map(brief => ({
    ...brief,
    articles: articles.slice(0, 3) // Use current articles for now
  })) : fallbackBriefTypes;
    ...
    <div className="bg-surface-elevated-light dark:bg-surface-elevated-dark rounded-lg p-6 border border-gray-200 dark:border-gray-700">
      ...
      <div className="space-y-3">
        {(brief.articles || []).map((article, index) => (
          <div key={article.id || index} className="flex gap-3">
