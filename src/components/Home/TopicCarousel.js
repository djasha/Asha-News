...
const TopicCarousel = ({ articles = [] }) => {
  ...
  // Load topic categories from CMS
  useEffect(() => {
    const loadTopicSections = async () => {
      try {
        const categories = await topicCategoriesService.getMainCategories();
        ...
        const sections = categories.map(category => ({
          ...
          articles: articles.filter(a => 
            a.category?.toLowerCase().includes(category.slug?.toLowerCase()) || 
            a.tags?.some(tag => tag.toLowerCase().includes(category.slug?.toLowerCase()))
          ).slice(0, 4)
        }));
