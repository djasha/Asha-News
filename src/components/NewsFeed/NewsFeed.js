...
import SimpleArticleCard from "../ArticleCard/SimpleArticleCard";
...
const NewsFeed = ({
  ...
  const [directusArticles, setDirectusArticles] = useState([]);
  ...
  // Fetch both RSS and Directus articles on component mount
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        // Fetch articles from Directus CMS
        const rssData = await directusService.getArticles({ sortBy: 'date' });
        setRssArticles(rssData.articles);
        console.log(`Loaded ${rssData.articles.length} RSS articles`);
        
        // Fetch Directus articles
        const directusData = await directusService.getArticles({ sortBy: 'date', limit: 50 });
        setDirectusArticles(directusData.articles);
        console.log(`Loaded ${directusData.articles.length} Directus articles`);
        
        // Fetch Palestine articles
        const palestineData = await directusService.getPalestineArticles({ limit: 10 });
        if (palestineData.articles.length > 0) {
          setDirectusArticles(prev => [...prev, ...palestineData.articles]);
          console.log(`Loaded ${palestineData.articles.length} Palestine articles`);
        }
  ...
  // Combine RSS and Directus articles, prioritizing prop articles if provided
  const allArticles = useMemo(() => {
    const combined = [...rssArticles, ...directusArticles];
    // Sort by publication date (newest first)
    combined.sort((a, b) => new Date(b.publication_date) - new Date(a.publication_date));
    console.log("Combined articles:", combined.length, "RSS:", rssArticles.length, "Directus:", directusArticles.length);
    return combined;
  }, [rssArticles, directusArticles]);

  const articles = propArticles.length > 0 ? propArticles : allArticles;
  console.log("NewsFeed using articles:", articles.length, propArticles.length > 0 ? "(props)" : "(combined)");
        ...
        <div className="transition-opacity duration-300 ease-in-out">
          <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
            ...
            <div className="flex-1 space-y-6 lg:space-y-8">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
                    {articlesWithImages.slice(0, isHomePage ? 6 : articlesWithImages.length).map((article) => (
                      <ErrorBoundary key={article.id} showDetails={true}>
                        <SimpleArticleCard
                          article={article}
                        />
                      </ErrorBoundary>
              ...
              {/* Palestine News Section */}
              {(() => {
                const palestineArticles = articlesWithImages.filter(a => a.topic === "Palestine");
                console.log("Palestine articles with images:", palestineArticles.length, palestineArticles.map(a => a.title));
                return palestineArticles.length > 0;
              })() && (
                <div>
                  <div className="flex items-center justify-between mb-4 lg:mb-6">
                    <h3 className="text-lg font-bold text-text-primary-light dark:text-text-primary-dark pb-2 border-b border-primary-200 dark:border-primary-700">
                      🇵🇸 Palestine - Forensic Investigations
                    </h3>
                    {!isHomePage && articlesWithImages.filter(a => a.topic === "Palestine").length > 4 && (
                      <button
                        onClick={() => navigate('/news/palestine')}
                        className="text-sm font-medium text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 transition-colors"
                      >
                        Read More →
                      </button>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
                    {articlesWithImages
                      .filter((article) => article.topic === "Palestine")
                      .slice(0, isHomePage ? 4 : articlesWithImages.filter(a => a.topic === "Palestine").length)
                      .map((article) => (
                        <SimpleArticleCard
                          key={article.id}
                          article={article}
                  ...
                  {isHomePage && articlesWithImages.filter(a => a.topic === "Palestine").length > 4 && (
                    <div className="mt-6 text-center">
                      ...
                      >
                        View All Palestine Investigations ({articlesWithImages.filter(a => a.topic === "Palestine").length - 4} more)
                      </button>
                    </div>
                  )}
                </div>
