...
class RSSAdapter {
  ...
  async fetchArticles(params = {}) {
    ...
    if (countries) {
      ...
      logger.debug(`Excluded sources from countries '${countries}':`, sources.map(s => `${s.name} (${s.country})`));
      ...
      if (sources.length === 0) {
        ...
        logger.debug('No sources after country filter, using international sources:', sources.map(s => s.name));
      }
    }
    ...
    try {
      ...
      logger.info(`Fetched ${allArticles.length} total articles from ${sources.length} sources`);
      ...
      if (keywords && keywords.trim()) {
        ...
        logger.debug(`Filtered by keywords '${keywords}': ${filteredArticles.length} articles`);
        
        // If no articles found with keywords, try broader search
        if (filteredArticles.length === 0) {
          logger.debug('No articles found with exact keyword match, trying broader search...');
          filteredArticles = allArticles.filter(article => {
            const searchText = `${article.title} ${article.summary} ${article.content}`.toLowerCase();
          ...
          logger.debug(`Broader keyword search found: ${filteredArticles.length} articles`);
        }
      }
      ...
      if (category && category !== 'general') {
        ...
        logger.debug(`Filtered by category '${category}': ${filteredArticles.length} articles`);
      }
      ...
      logger.debug(`Final result: ${finalArticles.length} articles after limit`);
      
      return finalArticles;
    } catch (error) {
      logger.error('RSS Adapter Error:', error);
      throw error;
    }
  }
  ...
  normalizeArticle(item, source) {
    try {
      ...
      logger.warn('Error normalizing article:', error);
      return null;
    }
  }
