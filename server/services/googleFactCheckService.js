...
class GoogleFactCheckService {
  ...
  async searchClaims(query, options = {}) {
    ...
    try {
      ...
      logger.info('Google Fact Check API URL:', url);
      
      const response = await fetch(url);
      
      if (!response.ok) {
        const errorText = await response.text();
        logger.error('Google Fact Check API error response:', errorText);
      ...
      logger.error('Google Fact Check search error:', error);
      throw error;
    }
  }
  ...
  async searchClaimsByImage(imageUri, options = {}) {
    ...
    try {
      ...
      logger.error('Google Fact Check image search error:', error);
      throw error;
    }
  }
