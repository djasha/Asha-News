...
// Database query bridge (replaces Directus HTTP client)
...
router.post('/claims', async (req, res) => {
  try {
    ...
    logger.error({ err: error }, 'Error submitting claim');
    res.status(500).json({
...
/**
 * POST /api/fact-check/checkArticle
 * Fetch an article from Directus, run AI analysis, and write results back
 * Body: { article_id: string }
 */
router.post('/checkArticle', async (req, res) => {
  try {
    ...
    // 1) Fetch article from Directus
    const articleResp = await directusFetch(`/items/articles/${article_id}`);
    const article = articleResp?.data;
    if (!article) {
      return res.status(404).json({ error: 'Article not found' });
    }

    // 2) Run AI analysis on article content (fallback to title)
    const text = article.content || article.summary || article.title || '';
    const analysis = await aiProviderService.analyzeClaim(text, 'auto', []);

    // Best-effort extraction of fields from provider response
    const patch = {
      political_bias: analysis?.political_bias || analysis?.bias || null,
      factual_quality: typeof analysis?.factual_quality === 'number' ? analysis.factual_quality : (analysis?.quality || null),
      confidence_score: typeof analysis?.confidence_score === 'number' ? analysis.confidence_score : (analysis?.confidence || 0.5)
    };

    // 3) Update article back into Directus
    await directusFetch(`/items/articles/${article_id}`, {
      method: 'PATCH',
      body: JSON.stringify(patch)
    });

    return res.json({ updated: true, article_id, patch, ai_analysis: analysis });
  } catch (error) {
    logger.error({ err: error }, 'checkArticle error');
    return res.status(error.status || 500).json({ error: error.message || 'Failed to analyze article' });
  }
});
...
router.get('/claims/:id', async (req, res) => {
  try {
    ...
    logger.error({ err: error }, 'Error getting claim');
    res.status(500).json({
...
router.post('/analyze', async (req, res) => {
  try {
    const { claim, provider = 'perplexity', evidence = [], model = 'llama-3.1-sonar-small-128k-online' } = req.body;
    ...
    // Use Perplexity AI for fact-checking analysis
    if (provider === 'perplexity') {
      const perplexityResponse = await aiProviderService.analyzeWithPerplexity(claim, model, evidence);
      return res.json({
        claim_id: require('uuid').v4(),
        claim,
        analysis: perplexityResponse.content || perplexityResponse.message,
        sources: perplexityResponse.citations || [],
        confidence: perplexityResponse.confidence || 0.8,
        timestamp: new Date().toISOString()
      });
    }

    // Fallback to existing AI provider service
    ...
    logger.error({ err: error }, 'Error analyzing claim');
    res.status(500).json({
 ...
 * Search using Perplexity AI for fact-checking
 */
router.post('/perplexity-search', async (req, res) => {
  try {
    ...
    // Use Perplexity AI for search
    const searchResults = await aiProviderService.searchWithPerplexity(query, model);
    ...
    logger.error({ err: error }, 'Error searching with Perplexity');
    res.status(500).json({
      error: 'Failed to search with Perplexity AI'
    });
  }
});
...
router.post('/evidence', async (req, res) => {
  try {
    const { search_terms, sources = ['news_api'] } = req.body;
    ...
    logger.error({ err: error }, 'Error searching evidence');
    res.status(500).json({
...
router.post('/credibility', async (req, res) => {
  try {
    ...
    logger.error({ err: error }, 'Error checking credibility');
    res.status(500).json({
...
router.post('/complete', async (req, res) => {
  try {
    ...
    logger.error({ err: error }, 'Error performing complete fact-check');
    res.status(500).json({
...
router.get('/google-search', async (req, res) => {
  try {
    ...
    logger.error({ err: error }, 'Error searching fact checks');
    res.status(500).json({
...
router.post('/google-image-search', async (req, res) => {
  try {
    ...
    logger.error({ err: error }, 'Error searching Google fact-check by image');
    res.status(500).json({
...
router.get('/recent-claims', async (req, res) => {
  try {
    ...
    logger.error({ err: error }, 'Error getting recent claims');
    res.status(500).json({
